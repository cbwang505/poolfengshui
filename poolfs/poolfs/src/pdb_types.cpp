...
void PDBTypes::print_types_by_name(const char* name)
{
	puts("******* TPI list of types (parsed types) *******");
	if (!parsed)
	{
	...
	for (PDBTypeDefNameMap::iterator it = types_byname.begin(); it != types_byname.end(); ++it)
	{
		if (it->second != nullptr&& it->first.compare(name)==0)
		{
			it->second->dump();
			puts("");
		}
...
int PDBTypes::get_types_field_offset(const char* name, const char* field)
{
	//puts("******* TPI list of types (parsed types) *******");
	if (!parsed)
	{
	...
	for (PDBTypeDefNameMap::iterator it = types_byname.begin(); it != types_byname.end(); ++it)
	{
		if (it->second != nullptr && it->first.compare(name) == 0)
		{
			PDBTypeStruct* sct = reinterpret_cast<PDBTypeStruct*>(it->second);
			if (sct->struct_members.size() > 0)
			{
				
				for (unsigned int i = 0; i < sct->struct_members.size(); i++)
				{

					if(_stricmp(sct->struct_members[i]->name, field)==0)
					{
						return	sct->struct_members[i]->offset;
					}					
				}
