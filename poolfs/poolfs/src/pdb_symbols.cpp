...
void PDBSymbols::parse_symbols(void)
{
	...
	while (unsigned(position) < pdb_sym_size)
	{
		...
		if (symbol->type == S_GDATA32 || symbol->type == S_LDATA32|| symbol->type == S_PUB32)
		{  // Global variable
...
PDBFunction* PDBSymbols::get_function_by_name(const char* name)
{
	for (PDBFunctionAddressMap::iterator it = functions.begin(); it != functions.end(); ++it)
	{
		if (it->second != nullptr)
...
PDBGlobalVariable* PDBSymbols::get_global_variable_by_name(const char* name)
{
...
void PDBSymbols::print_global_variable(PDBGlobalVariable* val)
{
	printf("Global variable [%s] at 0x%llx\n", val->name, val->address);
	printf("\tOffset : %08x\n", val->offset);
	printf("\tSection: %04x\n", val->section);
	printf("\tModule : %d\n", val->module_index);
	printf("\tType   : %08x ", val->type_index);
	int size = -1;
	if (val->type_def != nullptr)
	{
		val->type_def->dump(true);
		size = val->type_def->size_bytes;
	}
	puts("");
	printf("\tSize   : %d bytes\n", size);
	puts("");
...
void PDBSymbols::print_global_variables(void)
{
	...
	for (PDBGlobalVarAddressMap::iterator it = global_variables.begin(); it != global_variables.end(); ++it)
	{
		print_global_variable(&it->second);
	}
}
